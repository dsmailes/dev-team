...
7. Record `Source Isolation` before mutation or build work. Use `isolated` for concurrent tickets when branches/worktrees are available; otherwise use `serialized`, granting one mutating/building ticket exclusive shared-worktree ownership at a time. Use a separate build/cache path when the platform has mutable build state, such as Xcode DerivedData.
...
13. Require the executor to commit the ticket's scoped changes, record the base and ticket SHA, clean status, workspace, and artifact root before review.
...
2. Give the reviewer the ticket path, diff context, ticket commit SHA, and a clean verification worktree checked out at that SHA.
...
2. Give the tester the ticket path, expected verification scope, reviewed commit SHA, and a clean verification worktree checked out at that SHA.
...
Parallelize only when tickets have disjoint ownership and isolated branches/worktrees. If isolated worktrees are unavailable, serialize mutating and building tickets; read-only investigations may still run in parallel.

For every concurrent ticket:

1. Create a dedicated branch and executor worktree before assigning the ticket.
2. Record the base commit, worktree path, branch, and ticket-scoped artifact root in `Source Isolation`.
3. Use isolated mutable build state where needed, for example a unique DerivedData path for each Xcode worktree.
4. Commit the scoped implementation and record its SHA and clean status.
5. Create a clean verification worktree checked out at that exact SHA; Reviewer and Tester verify only there.
6. If a fix creates a new SHA, repeat the applicable review and test gates for that SHA.
7. Merge only reviewed, tested ticket commits into an integration branch. Record batch membership and merge/conflict resolution, then run the full integration matrix once before main.
8. Clean up only merged, verified ticket worktrees and branches after artifact capture. Preserve blocked or unmerged workspaces for diagnosis.
9. To abandon unmerged work, preserve its commit and remove only the named ticket workspace after a clean/ownership check. To undo integrated work, create a scoped revert commit; never reset shared history or remove another ticket's workspace.
...
- The tickets cannot be assigned separate worktrees, branches, or required mutable build/cache paths; use serialized execution instead.
