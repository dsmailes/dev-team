...
- Treat `.tickets/*.md` plus `.tickets/queue.md` as the only authoritative live board. Worker logs and generated dashboard files are not ticket state.
- Keep `## State` to one exact lifecycle token; put closure or blocker prose in a separate section.
...
- After every ticket or queue mutation, run `python3 scripts/render-ticket-dashboard.py --validate`, fix any mismatch, and then run `python3 scripts/render-ticket-dashboard.py` to refresh both generated projections.
- Never display or summarize `docs/tickets.html` or `docs/tickets.md` unless they were regenerated after the latest board mutation. For ordinary status, read the live queue and ticket files directly.
