...
- Fill in `Source Isolation`: select `isolated` for concurrent mutating/building work when branches/worktrees are available, otherwise select `serialized` and grant exclusive shared-worktree ownership. Record base commit, executor workspace, ticket-scoped artifact root, and require a ticket commit SHA before review.
...
Honor the ticket's `Source Isolation`. In `isolated` mode, edit only in the assigned worktree and use its isolated build/cache path. In `serialized` mode, own the shared worktree exclusively while mutating/building. Commit scoped changes and record base SHA, ticket SHA, clean status, workspace, and artifact root before handoff.
...
Review the ticket's recorded commit SHA in a clean verification worktree against the acceptance criteria. Do not rely on previous chat history or a moving shared checkout; use the ticket, supplied diff context, clean status, and exact commit.
...
Verify the reviewed commit SHA independently in a clean verification worktree. Do not verify a moving shared checkout; record commit identity, clean status, and the ticket-scoped artifact root.
